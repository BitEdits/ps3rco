...
#endif
// following doesn't seem to work - probably the define is required in the
// project

/* #ifdef _MSC_VER #define _CRT_SECURE_NO_WARNINGS // get rid of MSVC's
 * annoying security stuff #define _CRT_NON_CONFORMING_SWPRINTFS // urgh...
 * seems that mingw uses bad swprintf >_> #endif */
...
strwcpy (wchar_t * dest, const char *src, unsigned int maxLen)
{
...
#define READ_TEXT_SECTION(to, tl, te, typ, var, pVar, nam) \
  ...
  READ_TEXT_SECTION (header.textOffset, header.textLength, header.textEntries,
      wchar, out->text, out->pText, "text");
  READ_TEXT_SECTION (header.propOffset, header.propLength, header.propEntries,
      wchar, out->prop, out->pProp, "properties");
  READ_TEXT_SECTION (header.namesOffset, header.namesLength,
      header.namesEntries, char, out->names, out->pNames, "names");
...
readVSMXMem (const void *in)
{
  ...
  if (header->codeOffset != sizeof (VSMXHeader)) {
    warning ("Skipping range 0x%x-0x%x", sizeof (VSMXHeader),
	header->codeOffset);
    // fseek(fp, header->codeOffset, SEEK_SET);
  }
...
#define READ_TEXT_SECTION_MEM(to, tl, te, typ, var, pVar, nam) \
  ...
  READ_TEXT_SECTION_MEM (header->textOffset, header->textLength,
      header->textEntries, wchar, out->text, out->pText, "text");
  READ_TEXT_SECTION_MEM (header->propOffset, header->propLength,
      header->propEntries, wchar, out->prop, out->pProp, "properties");
  READ_TEXT_SECTION_MEM (header->namesOffset, header->namesLength,
      header->namesEntries, char, out->names, out->pNames, "names");
...
void *
writeVSMXMem (unsigned int *len, VsmxMem * in)
{
  *len =
      sizeof (VSMXHeader) + in->codeGroups * sizeof (VSMXGroup) + in->lenText +
      in->lenProp + in->lenNames;
  void *ret = malloc (*len);
...
VsmxDecode (VsmxMem * in, FILE * out)
{
  ...
  for (i = 0; i < in->codeGroups; i++) {
	...
	if (in->code[i].val.u32 == 1)
	  ...
	  warning ("Unexpected boolean value 0x%x at line %d!",
	      in->code[i].val.u32, i + 1);
...
unsigned int
VsmxAddText (void **text, unsigned int **offs, unsigned int *textLen,
    unsigned int *numText, wchar * newText, int charWidth)
{
...
VsmxEncode (FILE * in)
{
  ...
  ret->codeGroups = ret->numText = ret->numProp = ret->numNames = ret->lenText =
      ret->lenProp = ret->lenNames = 0;
  ret->code = NULL;
  ret->text = NULL;
  ret->prop = NULL;
  ret->names = NULL;
	...
	*tmp = L'\0';
      ...
      case VID_DEBUG_FILE:
	{
	  if (arg[0] != '"' || arg[wcslen (arg) - 1] != '"') {
	    ...
	    arg[wcslen (arg) - 1] = '\0';
	    argNum =
		VsmxAddText ((void **) &ret->text, &oText, &ret->lenText,
		&ret->numText, arg, sizeof (*(ret->text)));
	  }
	}
	break;
      case VID_VARIABLE:
	argNum =
	    VsmxAddText ((void **) &ret->names, &oNames, &ret->lenNames,
	    &ret->numNames, arg, sizeof (*(ret->names)));
	break;
      case VID_PROPERTY:
      case VID_METHOD:
      case VID_UNK_31:
	...
	// ops w/o arg - check for zero
      ...
      case VID_END:
	if (arg)
	  warning ("[line %d] Operator does not have value.", lineCount);
	break;
    ...
    ret->codeGroups++;
    ret->code =
	(VSMXGroup *) realloc (ret->code, ret->codeGroups * sizeof (VSMXGroup));
...
static inline void
VsmxDecompileStackPush (VsmxDecompileStack ** stack, VsmxDecompileStack * item)
{
  VsmxDecompileStack *newItem =
      (VsmxDecompileStack *) malloc (sizeof (VsmxDecompileStack));
  *newItem = *item;
  newItem->prev = *stack;
  if (*stack)
...
static inline VsmxDecompileStack *
VsmxDecompileStackPop (VsmxDecompileStack ** stack)
{
  if (!(*stack)) {
...
static inline void
VsmxDecompileStackDestroy (VsmxDecompileStack ** stack)
{
  if (!*stack)
    return;
  while (*stack) {
...
static inline void
VsmxDecompMarkStackPush (VsmxDecompMarkStack ** stack,
    VsmxDecompMarkStack * item)
{
  VsmxDecompMarkStack *newItem =
      (VsmxDecompMarkStack *) malloc (sizeof (VsmxDecompMarkStack));
  *newItem = *item;
  newItem->prev = *stack;
  if (*stack) {
    newItem->depth = (*stack)->depth + 1;
...
static inline VsmxDecompMarkStack *
VsmxDecompMarkStackPop (VsmxDecompMarkStack ** stack)
{
  if (!(*stack)) {
...
static inline void
VsmxDecompMarkStackDestroy (VsmxDecompMarkStack ** stack)
{
  if (!*stack)
    return;
  while (*stack) {
...
VsmxDecompile (VsmxMem * in, FILE * out)
{
  ...
  for (i = 0; i < in->codeGroups; i++) {
    ...
    if (in->code[i].id >> 8 && (in->code[i].id & 0xFF) != VID_FUNCTION) {
      warning ("Unexpected flags 0x%x for id 0x%x at %d", in->code[i].id >> 8,
	  in->code[i].id & 0xFF, i);
    }
    // check for markers
	...
	if (in->code[mStack->src].id == VID_JUMP_FALSE) {
	  ...
	  if (in->code[i].id == VID_JUMP_FALSE) {
	    // this upcomming if acts as an AND - we'll display this as a
	    // nested if though
	...
	if (indent > 0)
	  indent--;		// this should always be true
	else
	  warning ("Internal state nesting error!");
     ...
     * MAX_TEXT_LEN, L##s, __VA_ARGS__) #endif */
#define SWPRINTF_ITEM(s, ...) SWPRINTF(item.str, MAX_TEXT_LEN, L##s, __VA_ARGS__)
    // #define SWPRINTF_ITEM(s, ...) swprintf(item.str, MAX_TEXT_LEN, L s,
    // __VA_ARGS__)
	...
	{			// TODO: need to consider when to use brackets
	  ...
	  // swprintf(item.str, MAX_TEXT_LEN, L"%s %s %s", left->str, op,
	  // right->str);
	  VsmxDecompileStackPush (&stack, &item);
	  free (left);
	  free (right);
	...
	break;
      case VID_CONST_FLOAT:
	// swprintf(item.str, MAX_TEXT_LEN, L"%f",
	// *(float*)(&in->code[i].val.u32));
	...
	{			// TODO: unicode issues... :(
	  CHECK_INDEX (in->numText, "text");
	  // swprintf(item.str, MAX_TEXT_LEN, L"\"%s\"",
	  // in->pText[in->code[i].val.u32]);
	...
	break;
      case VID_UNNAMED_VAR:
	// swprintf(item.str, MAX_TEXT_LEN, L"__GLOBALS__[%u]",
	// in->code[i].val.u32);
	...
	if (stack && stack->depth >= 1) {	// TODO: this is really not
	  ...
	  SWPRINTF_ITEM ("%ls.%ls = %ls", obj->str,
	      in->pProp[in->code[i].val.u32], val->str);
	  free (obj);
	  free (val);
	} else
      ...
      case VID_UNSET:
	{
	  ...
	  // swprintf(item.str, MAX_TEXT_LEN, L"%s.%s", prev->str,
	  // in->pProp[in->code[i].val.u32]);
	  if (in->code[i].id == VID_UNSET)
	    SWPRINTF_ITEM ("delete %ls.%ls", prev->str,
		in->pProp[in->code[i].val.u32]);
	  else
	    SWPRINTF_ITEM ("%ls.%ls", prev->str,
		in->pProp[in->code[i].val.u32]);
	  VsmxDecompileStackPush (&stack, &item);
	  free (prev);
	}
      ...
      case VID_STACK_PUSH:
	{
	  ...
	  // !!! look ahead used!
	  if (i + 1 < in->codeGroups && (in->code[i + 1].id == VID_JUMP_FALSE ||
		  in->code[i + 1].id == VID_JUMP_TRUE)) {
	    // 1st condition (hoping there isn't a more complex situation) -
	    // ignore
	  } else {
      ...
      case VID_FUNCTION:
	{
	  ...
	  for (argI = 0; argI < numArgs; argI++) {
	    if (argI)
	      wcscat (args, L", ");
	    SWPRINTF (args + wcslen (args), 4096 - wcslen (args), L"__var%d",
		argI + 1);
	  }

	  if (i + 2 < in->codeGroups &&
	      in->code[i + 1].id == VID_OPERATOR_ASSIGN && (funcEndStmtStyle ||
		  (in->code[i + 2].id == VID_SECT_START &&
		      in->code[i].val.u32 == i + 3))) {
	    // abc = function() style?

	    if (in->code[i + 1].val.u32 || (funcEndStmtStyle &&
		    in->code[i + 2].val.u32)) {
	      warning ("Unexpected values in function definition style at %d",
		  i);
	    }

	    if (!stack) {
	      warning
		  ("Function encountered at %d, but no name found on stack!",
		  i);
	      ...
	      SWPRINTF_ITEM ("%ls = function %ls(%ls)", var->str, prev->str,
		  args);
	      VsmxDecompileStackPush (&stack, &item);
	      free (prev);
	      free (var);
	      ...
	      warning
		  ("Function encountered at %d, but more than one item found on stack!",
		  i);
	      VsmxDecompileStack *prev;
	    ...
	    if (funcEndStmtStyle)
	      ...
	      i++;
	  } else if (i + 1 < in->codeGroups &&
	      in->code[i + 1].id == VID_SECT_START &&
	      in->code[i].val.u32 == i + 2) {
	    // function abc() style?
	    VsmxDecompileStack *prev;
      ...
      case VID_ARRAY_INDEX:
	{
	  ...
	  // TODO: check that the number supplied in in->code[i].val.u32 isn't
	  // used (always is 0)!
      ...
      case VID_CALL_NEW:
	{
	  if (stack->depth < in->code[i].val.u32) {
	    error
		("Not enough arguments to perform requested function call at group %d.",
		i);
	    return 1;
	  }
	  VsmxDecompileStack *prev;
	  ...
	  if (in->code[i].val.u32 > 0) {
		...
		// SWPRINTF_ITEM(", %ls", item.str);
		memmove (prev->str + 2, prev->str,
		    (wcslen (prev->str) + 1) * sizeof (wchar));
		// wcscat(prev->str, L", ");
		prev->str[0] = ',';
		prev->str[1] = ' ';
		wcscat (prev->str, item.str);
      ...
      case VID_JUMP_TRUE:
	{
	  ...
	  if (
	      ...
	      && in->code[i].val.u32 < in->codeGroups &&
	      in->code[in->code[i].val.u32 - 1].id == VID_SECT_START &&
	      ...
	      && in->code[i + 1].val.u32 < in->codeGroups &&
	      in->code[in->code[i + 1].val.u32 - 1].id == VID_SECT_START &&
	      in->code[in->code[i + 1].val.u32 - 1].val.u32 == i + 2) {
	    // assume for loop
	    SWPRINTF_ITEM ("for(; %ls /* jump to %d */; ", prev->str,
		in->code[i].val.u32);
	    VsmxDecompileStackPush (&stack, &item);
	    ...
	    // assume OR
	    SWPRINTF_ITEM ("( %ls ) || /* ends at %d */ ( ", prev->str,
		in->code[i].val.u32);
	    VsmxDecompileStackPush (&stack, &item);
	...
	if (!op[0]) {
	  ...
	  if (mStack && (in->code[mStack->src].id == VID_JUMP_FALSE ||
		  (in->code[mStack->src].id == VID_SECT_START &&
		      in->code[i].val.u32 < i))) {
	  ...
	  } else if (mStack && in->code[mStack->src].id == VID_JUMP_TRUE &&
	      in->code[mStack->src].val.u32 == i + 1) {
	    // for loop end of stmt probably
	    if (stack)
	      warning ("Unexpected stack at end of for at %d", i);
	...
	if (!op[0]) {
	  ...
	  if (in->code[in->code[i].val.u32 - 1].id == VID_SECT_START &&
		...
		free (prev);
	      ...
	      endStmtConcat = 2;
	      SWPRINTF_ITEM ("( %ls ? /* ends at %d */ ", tmp,
		  in->code[i].val.u32);
	      VsmxDecompileStackPush (&stack, &item);
	      mItem.src = i;
	      mItem.loc = in->code[i].val.u32;
	...
	if (!notSectStart) {
      ...
      case VID_RETURN:
	if (!op[0])
	  wcscpy (op, L"return %s;\n");
      case VID_END_STMT:
	if (!op[0])
	  wcscpy (op, L"%s;\n");
	{
	  // end of statement following conditional - skip
	  if (in->code[i].id == VID_END_STMT && i &&
	      (in->code[i - 1].id == VID_JUMP_TRUE ||
		  in->code[i - 1].id == VID_JUMP_FALSE))
	    break;

	  if (stack) {
	    if (stack->depth > endStmtConcat - 1) {
	      warning
		  ("End of statement but more stack elements than expected at groups %d-%d!",
		  stmtStart, i);
	    }
	    // TODO: remove debugging line
	    fwprintf (out, L"/*%d*/\t", stmtStart);
    ...
    /* if(is_value) { item.item = in->code[i]; VsmxDecompileStackPush(&stack,
     * &item); } */
  }
  return 0;
}
