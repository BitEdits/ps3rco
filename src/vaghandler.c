...
Bool vag2wav(const char* fWav, int numChannels, int* vagLen, void** vagData) {
	...
	for(i=0; i<numChannels; i++) {
		...
		// TODO: version check
		if(vh->signature != VAG_SIGNATURE || vh->frequency != wh.frequency) {
			free(vag);
			fclose(fp);
			return FALSE;
		}
	...
	// take a guess at the length (we'll fix these later if wrong) - make assumption that all channels are of same size
	wh.s2size = (((*vagLen - sizeof(VagHeader)) / sizeof(VagChunk)) -1) * 28/*samples*/ * 2/*16bit*/ * numChannels;
		...
		for(i=0; i<numChannels; i++) {
			...
			if(vc->flags != 7) {
				...
				for(j=0; j<28; j++) {
					...
					double sample = (double)(s >> shift) + factors[i*2] * f[predict][0] + factors[i*2+1] * f[predict][1];
...
#define WAV2VAG_ERROR_EXIT(s) { \
	...
	if(wh.s1size != WAV_S1_SIZE_PCM) {
		warning("File '%s' appears to have a non-standard header length for PCM.  Attempting to auto-skip...", fWav);
		fseek(fp, sizeof(wh) + (wh.s1size - WAV_S1_SIZE_PCM) - sizeof(uint32)*2, SEEK_SET);
		fileread(fp, &wh.s2sig, sizeof(uint32));
	}
	if(wh.s1sig != WAV_S1_SIG || wh.s2sig != WAV_S2_SIG) 
		WAV2VAG_ERROR_EXIT("File '%s' structure not supported by rcomage's VAG handler; conversion aborted.")
	if(wh.audioFmt != WAV_PCM_FORMAT || wh.bitDepth != 16)
		WAV2VAG_ERROR_EXIT("rcomage's VAG handler only supports 16-bit PCM data, file '%s' not converted.")
	
	
	if(wh.channels > 9)
		WAV2VAG_ERROR_EXIT("Channels number too high for '%s'.")
	
	//if(wh.s2size != wh.size-36 || wh.size+8 != filesize(fWav) || wh.sampleSize != wh.channels*wh.bitDepth/8)
	...
	*len = (numSamples/28 + (numSamples % 28 ? 2:1)) * sizeof(VagChunk) + sizeof(VagHeader);
	*vagData = calloc(1, *len * wh.channels);
	...
	for(pos=0; pos<numSamples; pos += 28) {
		...
		if(pos+28 > numSamples)
			memset(wavBuf + ((numSamples-pos) * (wh.bitDepth/8) * wh.channels / sizeof(int16)), 0, (28 - numSamples+pos) * (wh.bitDepth/8) * wh.channels);
		
		uint ch;
		for(ch=0; ch<wh.channels; ch++) {
			VagChunk* vc = (VagChunk*)((char*)(*vagData) + *len * ch + sizeof(VagHeader) + sizeof(VagChunk)*(pos/28));
			...
			for(j=0; j<5; j++) {
				...
				for(k=0; k<28; k++) {
					...
					if(sample > 30719.0) sample = 30719.0;
					if(sample < -30720.0) sample = -30720.0;
					predictBuf[j][k] = sample - s1[j] * f[j][0] - s2[j] * f[j][1];
					if(fabs(predictBuf[j][k]) > max) max = fabs(predictBuf[j][k]);
					s2[j] = s1[j];
					s1[j] = sample;
				}
				if(max < min) {
			...
			// pack
			// I don't understand it, but it seems that the second transformation is required to prevent a clipping sound effect although it should produce worse reconverts...
			int8 outBuf[28];
			for(k=0; k<28; k++) {
				double s_double_trans = predictBuf[predict][k] - factors2[ch*2] * f[predict][0] - factors2[ch*2+1] * f[predict][1];
			...
			/* {
				...
				for(j=0; j<28; j++) {
					int s = samples[j] << 12; // shift 4 bits to top range of int16
					if(s & 0x8000) s |= 0xFFFF0000;
					double sample = (double)(s >> shift) + factors2[ch*2] * f[predict][0] + factors2[ch*2+1] * f[predict][1];
					factors2[ch*2+1] = factors2[ch*2];
					factors2[ch*2] = sample;
					if(wavBuf[j*wh.channels + ch] != (int16)ROUND(sample)) {
						printf("a");
					}
				}
			} */
	...
	for(i=0; i<wh.channels; i++) {
		VagChunk* vc = (VagChunk*)((char*)(*vagData) + *len * (i+1) - sizeof(VagChunk));
		vc->predict_shift = 0;
		vc->flags = 7;
		memset(&vc->s, 0x77, 14);
	}
...
#define WAV2VAG_ERROR_EXIT(s) { \
	...
	if(wh.signature != WAV_SIGNATURE || wh.format != WAV_FORMAT) 
		WAV2VAG_ERROR_EXIT("File '%s' not a valid WAV file, conversion aborted.")
	if(wh.s1sig != WAV_S1_SIG || wh.s1size != WAV_S1_SIZE_PCM || wh.s2sig != WAV_S2_SIG) 
		WAV2VAG_ERROR_EXIT("File '%s' structure not supported by rcomage's VAG handler; conversion aborted.")
	if(wh.audioFmt != WAV_PCM_FORMAT || wh.bitDepth != 16)
		WAV2VAG_ERROR_EXIT("rcomage's VAG handler only supports 16-bit PCM data, file '%s' not converted.")
		...
		for(ch=0; ch<wh.channels; ch++) {
			...
			for(j=0; j<5; j++) {
				...
				for(k=0; k<28; k++) {
					...
					if(sample > 30719.0) sample = 30719.0;
					if(sample < -30720.0) sample = -30720.0;
					predictBuf[j][k] = sample - s1 * f[j][0] - s2 * f[j][1];
					if(fabs(predictBuf[j][k]) > max) max = fabs(predictBuf[j][k]);
					s2 = s1;
					s1 = sample;
				}
			...
			uint8 outBuf[28];
			for(k=0; k<28; k++) {
				double sample = predictBuf[predict][k] - factors2[ch*2] * f[predict][0] - factors2[ch*2+1] * f[predict][1];
				int sample2 = ((int)(sample * (1 << shift)) + 0x800) & 0xfffff000;
				if(sample2 > 32767) sample2 = 32767;
				if(sample2 < -32768) sample2 = -32768;
				outBuf[k] = (uint8)(sample2 >> 8);
				sample2 >>= shift;
				factors2[ch*2+1] = factors2[ch*2];
				factors2[ch*2] = (double)sample2 - sample;
			}
			for(k=0; k<14; k++)
				vc->s[k] = (outBuf[k*2+1] & 0xF0) | ((outBuf[k*2] >> 4) & 0xF);
		}
	}
