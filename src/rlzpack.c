...
#define ABS_INPUT_POS ((unsigned int)rlzi->input + rlzi->inputpos)
...
#define log2(x) (log((long double)(x)) / (long double)0.30102999566398119521373889472449)

typedef struct { // unk_421D80
	unsigned char literals[8 * 0xFF]; //grouped in 8 groups of 0xFF; group selected by has of input position, last byte and mode, the 0xFF part is based on the input byte with various shifts
	unsigned char distDescLong[0xF0]; // 30x8 (match >= 4)
	unsigned char distDescShort[0x40]; // 8x8 (match < 4)
	unsigned char distances[12 * 0x20]; // grouped in 0x20 based on bit position
	unsigned char lenDesc[0x40]; // 8 x 8; group selected by bit position, position is var_C from main function
	...
	int64_t rangeOffset;		// 0xCB0 (0x422A30) - only written to in sub_405810(), affected by write_bit and write_match_len
							// - appears to be a data queue - top 24 bits not used, next 8 bits->data to be written, next 8 bits->data mask; affected by CB8 in some way (CB8 is added to this)
	long rangeSize;			// 0xCB8
	char nextOutputByte;			// 0xCBC - some byte offset; 3rd byte in CB0, only ever used in sub_405810; guaranteed never to == 0xFF
	...
	float sgl_120C84;
	float sgl_120C88; // starts as 0.0625 and increments by that amount every time the dictionary coder fails to pack stuff
...
int rlzcompress(void *output, int inlen, void *input, unsigned char mode)
{
	if(!inlen) return -1; // sanity check
	 ...
	 */
	int var_C = 0; // var_C appears to do some tracking in regards to how well the dictionary coder is packing things?
	char realMode;
	...
	rlzi->bytesToWrite = 0;
	// no writeLock=0 - writeLock is _always_ assigned before calling the write function anyway
	...
	/* ^ 4 possible combinations for the above
	 *  if bits45 == 00 (should be) then eax => 0x80	(10000000)
	 *  if bits45 == 01 then eax => 0x60				(01100000)
	 *  if bits45 == 10 then eax => 0x40				(01000000)
	 *  if bits45 == 11 then eax => 0x20				(00100000)
	 */
	{
	...
	while(rlzi->inputpos < rlzi->inputlen) //loc_4060D0
	{
		...
		if(matchLen < 2) matchLen = 1;
		int sub_405B90_ret = write_match_len(matchLen -1, var_C);
		if(matchLen == 1)						// no match found
		{
			// write literal
			// the following code was previously sub_405A10( (((((unsigned int)rlzi->inputpos & 7) << 8) | lastInputByte) >> realMode) & 7 )
			int arg_ecx = ((
				 (((unsigned int)rlzi->inputpos & 7) << 8) | lastInputByte
				) >> realMode) & 7; // top bit in (rlzi->inputpos & 7) will actually be discarded - that is, (rlzi->inputpos & 3) should give the same result
			...
			for(; !(inputVal & 0x10000); inputVal <<= 1) // loop 8 times
				write_bit(
					(unsigned char*)((unsigned int)rlzi->literals + arg_ecx*0xFF + (inputVal >> 8)-1),
					(inputVal >> 7) & 1
				); // last arg (now deleted) is actually ebx, but ebx is 3 at this time
			
			// these values are only ever used if realMode==7, and only in find_match() function
			#define scast_405A10(v) ((long double)((unsigned int)(v)))
			// (int) cast added to make it consistent with good rlz packer, but unsure if this is correct...
			rlzi->sgl_120C84 += (float)(log2(scast_405A10(oldRangeSize) / scast_405A10(rlzi->rangeSize)) + (int)((rlzi->outputpos - oldOutputpos - 1)*8));
		...
		else
		{
			...
			// originally sub_405C10(dictSearchBack, sub_405B90(matchLen -1, var_C), matchLen)
			...
			int var_edi;
			i = 0;
			if((searchBackPlus1 & 0xFFFFFFFE) > 0) // if searchBackPlus1 has bits other than the first set... (that is, searchBackPlus1 != 0 && searchBackPlus1 != 1)
			{ // this condition _should_ always be true, as min dictSearchBack is 1, and with +1, min value for searchBackPlus1 is 2
			...
			int var_ebx = 1;
			int var_esi;
			do { // write packed thing - perhaps a length of next number descriptor?
				var_esi = ((sbSignifBit & var_edi) > 0 ? 1 : 0); // check if certain bit in ebp is set
				write_bit((unsigned char*)((unsigned int)&treeStore[var_ebx*8] + sub_405B90_ret), var_esi);
				var_edi >>= 1;
		...
		for(i=0; i<bytesLeftMinus3; i++) // goes thru all the bytes just written and pushes some things into the dictionary
		{
			unsigned int brHash = *(unsigned int*)(ABS_INPUT_POS +i) << 8;
			brHash %= RLZI_WINDOW_SIZE;
			
			rlzi->dictionary[(rlzi->inputpos+i) & RLZI_DICTIONARY_SIZE] = rlzi->window[brHash];
			rlzi->window[brHash] = rlzi->inputpos+i;
		}
		
		rlzi->inputpos += matchLen;
		lastInputByte = *(unsigned char*)(ABS_INPUT_POS-1);
		if(rlzi->outputpos == rlzi->inputlen) // is the compressed output larger than the original stream?
			return -1;
	}
	
	write_match_len(0xFF, var_C); // end marker or is this used for flushing data?
...
int write_match_len(unsigned char len, unsigned int arg_4) // arg_4 -> something that keeps track of no. past blocks packed with find_match() (always var_C from rlzcompress())
{
	// len is the length -1 (amount of data removed by dictionary coder), except when 0xFF (when called after loop has finished)
	int i;
	char flagTemp;
	...
	for(i=0; i<8; i++)
	{
		...
		write_bit((unsigned char*)((unsigned int)&rlzi->lenDesc[i*8] + arg_4), flagTemp);
		if(flagTemp == 0) break;
	}
	i--;
	if(i > 0) // --> identical to if(len != 0); this will only be true if len >= 1 (or len<0) - eg if data was packed
	{
		i--;
...
// arg_edx is likely to be a 8bit value; something to do with bit position in distance
// distance: but can be "len" when called from write_match_len
void write_special(unsigned char* unk0ptr, int num, int arg_edx)
//void write_special(int arg_4, int distance, int inputpos, int arg_ecx, int arg_edx)
{
	if(arg_edx > 255)
		printf("ASSUMPTION FAILURE\n");
	//int var_ebx = ((inputpos << (arg_edx & 0xFF)) & 3) + arg_4 + ((arg_ecx & 7)+arg_edx*8)*4;
	if(arg_edx>=3)
	{
		...
		if(arg_edx>=3)
		{
			...
			arg_edx += 3; // 2 (reverse previous subtraction), and +1 for the above loop
...
void flush_output(void)
{
	...
	// -- which means it can only be false if rlzi->rangeOffset == 0x00000000FF??????
	if((rlzi->rangeOffset >> 0x18) != 0xFF)
	{
		unsigned char byteOut = rlzi->nextOutputByte; // starts off as 0, then becomes 3rd byte of rlzi->rangeOffset
		rlzi->nextOutputByte = (rlzi->rangeOffset >> 0x18) & 0xFF;
		do {
			if(rlzi->outputpos == rlzi->inputlen)
				return;
			*(unsigned char*)((unsigned int)rlzi->output + rlzi->outputpos) = ((rlzi->rangeOffset >> 32) & 0xFF) + byteOut;
			rlzi->bytesToWrite--;
			rlzi->outputpos++;
			byteOut = 0xFF;
	...
	//loc_4058A4:
	rlzi->bytesToWrite++;
	//rlzi->rangeOffset = (arg_CB0 << 8) & 0xFFFFFFFF; // make sure high 32 bits are 0
...
// NEW: this function searches for distance-length, but doesn't actually encode anything
int find_match(int *searchBackUsed, unsigned char realMode)
{
	int tempCompare = 0;
	int maxBlockSize = 0xFF, matchLen = 1, backRef;
	
	unsigned int bytesLeft = rlzi->inputlen - rlzi->inputpos;
	unsigned int searchBack; // not too sure about this variable's name
	...
	if(bytesLeft < 2)
		return 1;
	if(bytesLeft < 0xFF) // if we have less bytes left than the maximum block size
		maxBlockSize = bytesLeft;
	
	//realMode7Threshold = ((unsigned int)(rlzi->sgl_120C84 / rlzi->sgl_120C88 + 128.5) > 0x64); //dbl_41A3B8
	//realMode7Threshold = ((int)((unsigned int)(rlzi->sgl_120C84 / rlzi->sgl_120C88 + 128.5)) > 0x64);
	#define ROUND(n) ((int)((n) + 0.5))
	realMode7Threshold = (ROUND(rlzi->sgl_120C84 / rlzi->sgl_120C88) > -28);
	// the second line above generates output consistent with the good rlzpacker, but is this typecasting all correct?
	
	// check if we have a continuous copy of something
	searchBack = ABS_INPUT_POS - rlzi->lastSearchBack; // byte_CBF only used by find_match(); is initially 0
	if(
		   (unsigned int)rlzi->lastSearchBack < rlzi->inputpos
	...
	// no continuous copy from previously found stream - check if any similarities in the past few (up to 3) bytes
	...
	//loc_405DB7:
	// if we have 2 bytes left in the buffer (remember, if there's 1 byte left, this function would've
	//  already returned), we'll either by able to pack 1 or 2 bytes into a single byte
	...
	unsigned int brHash = *(unsigned int*)ABS_INPUT_POS << 8; // previously eax
	
	backRef = rlzi->window[(unsigned int)brHash % RLZI_WINDOW_SIZE];
	while(backRef != -1)
	{ // found a back reference
		if(backRef < (int)(rlzi->inputpos - RLZI_DICTIONARY_SIZE-1)) // is the backreference out of bounds?
			break;
		...
		{
			if(*(unsigned char*)((unsigned int)rlzi->input + backRef+i) != *(unsigned char*)(ABS_INPUT_POS+i))
				goto loc_405F97; // jumps to end of while-loop (not valid - search for more references which could be valid)
		}
		...
		for(; matchAmt < maxBlockSize; matchAmt++)
		{
			//loc_405E70:
			if(
				*(unsigned char*)(((unsigned int)rlzi->input + matchAmt)+backRef) !=
				*(unsigned char*)(((unsigned int)rlzi->input + matchAmt)+rlzi->inputpos)
			)
				break;
		}
		i = rlzi->inputpos - backRef - 1; // distance?  penalising short matches which have a large distance??
		if(matchAmt >= 3 && (matchAmt != 3 || (i <= 0x7F && (i <= 0x30 || realMode != 5))))
		{
			tempCompare=0;
			switch(matchAmt)
			{
				case(3): tempCompare = 0x7E;    break;
				case(4): tempCompare = 0x4400;  break;
				case(5): tempCompare = 0x6A00;  break;
				case(6): tempCompare = 0x28400; break;
			}
				...
				*searchBackUsed = i;
				if(*searchBackUsed)
				{
					...
					for(i=0; i < matchAmt; i++) {
						if(*(unsigned char*)((unsigned int)rlzi->input+backRef) != *(unsigned char*)(ABS_INPUT_POS + i))
							break;
					}
					if(matchAmt == i)
					{ // all matched is all the same character!
						...
						if(i == maxBlockSize)
						{
							...
							// loc_405F60:
							for(i=3; i < matchAmt; i++) // loop max of 6 times due to code below
							{
								if(i != 3 || (*searchBackUsed <= 0x7F && (*searchBackUsed <= 0x30 || realMode != 5)))
								{
									switch(i)
									{
										case(3): tempCompare = 0x7E;    break;
				...
				if(matchAmt == maxBlockSize)
				{
					...
					if(bytesLeft <= 0xFF) // if bytesLeft < 0xFF, then maxBlockSize < 0xFF will be true (enforced above), thus this line is essentially "if(bytesLeft == 0xFF)"
					{
						return 0xFD; // or 0x100 - 3
					}
					//loc_406030:
					// note, at this point, matchAmt = maxBlockSize = 0xFF, bytesLeft > 0xFF
					if(bytesLeft == 0x100) return 0xFD;
					for(i = matchAmt; i < 0x101; i++)
						if(*(unsigned char*)(ABS_INPUT_POS+i+backRef-rlzi->inputpos) != *(unsigned char*)(ABS_INPUT_POS+i))
							return 0xFD;
					return 0xFF;
				}
			}
