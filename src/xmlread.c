...
void parse_entry(xmlNodePtr node, rRCOEntry* entry, rRCOFile* rco, rcoxml_read_fixes* fixes);
//Bool rcoxml_text_to_int(char* s, const RcoTableMap map, uint* out);
uint rcoxml_add_label(char** labels, uint* labelsLen, char* label, Bool eventQuirk);
uint rcoxml_add_label_reordering(char* newLabels, uint* labelPos, char* label);
void rcoxml_reorder_labels(char* newLabels, uint* labelPos, rRCOFile* rco, rRCOEntry* entry);
...
void rcoxml_fix_ptrs(rRCOEntry*** sect, uint* sectCnt, rRCOFile* rco, const char* text);
...
rRCOFile* read_xml(char* fn) {
	...
	//fixes.ptrText = fixes.ptrImg = fixes.ptrSound = fixes.ptrModel = fixes.ptrObj = fixes.ptrAnim = NULL;
	fixes.textData = NULL;
	//fixes.refsCnt = 0;
	//fixes.refs = (xmlrco_read_fix_refs*)malloc(1); // dummy malloc to allow realloc to work later on
	...
	if((node = xmlDocGetRootElement(doc)))
		bValidDoc = (xmlStrcmp(node->name, _X("RcoFile")) ? FALSE : TRUE);
	if(bValidDoc) bValidDoc = (node->xmlChildrenNode ? TRUE : FALSE);
	
	xmlNodePtr nodeChild;
	if(bValidDoc) { // valid document must contain at least one thing under RcoFile (MainTree)
	...
	//rco->numPtrText = rco->numPtrImg = rco->numPtrModel = rco->numPtrSound = rco->numPtrObj = rco->numPtrAnim = 0;
	rco->tblVSMX = rco->tblText = rco->tblImage = rco->tblSound = rco->tblModel = rco->tblFont = rco->tblObj = rco->tblAnim = NULL;
	...
	if(verId) {
		...
		if(sscanf((const char*)verId, "unknownId%i", &verIdInt))
			rco->verId = verIdInt;
		else if(!xmlStrcmp(verId, _X("ps3"))) // for compatibility
			rco->verId = 0x107;
			...
			if(verIdFlt < 1.0) {
				...
				warning("[line %d] Unknown ID for firmware version '%f'.", node->line, verIdFlt);
			}
		}
		else
	...
	// XML version
	xmlChar* xmlVer = xmlGetProp(node, _X("rcomageXmlVer"));
	if(xmlVer) {
		float xmlVerFlt;
		if(sscanf((const char*)xmlVer, "%f", &xmlVerFlt)) {
			if(xmlVerFlt > APPVER)
				warning("This XML file was generated by a newer version (v%f) of rcomage\nand may not read properly with this version.", xmlVerFlt);
		}
		else
			warning("[line %d] Bad value for 'rcomageXmlVer'.\n  So... do you have a reason for fiddling with this?? >_<", node->line);
		xmlFree(xmlVer);
	}
	
	rco->umdFlag = 0; // assumed default
	xmlChar* umdFlag = xmlGetProp(node, _X("UMDFlag"));
	if(umdFlag) {
	...
	rco->ps3 = FALSE;
	xmlChar* ps3Flag = xmlGetProp(node, _X("type"));
	if(ps3Flag) {
		if(!xmlStrcmp(ps3Flag, _X("ps3")))
			rco->ps3 = TRUE;
		else if(xmlStrcmp(ps3Flag, _X("psp")))
	...
	/* rco->labels = (char*)malloc(1);
	rco->labels[0] = '\0';
	rco->events = (char*)malloc(1);
	rco->events[0] = '\0'; */
	...
	/*
	rco->tblMain.id = rco->tblMain.type = 0;
	rco->tblMain.labelOffset = 0;
	rco->tblMain.numSubentries = rco->tblMain.extraLen = 0;
	*/
	
	parse_entry(nodeChild, &(rco->tblMain), rco, &fixes);
	// TODO: ensure the first table _is_ the main table (and possibly then verify tree structure)
	...
	// label reordering - really optional, but does help to make RCOs similar to Sony's
	{
		...
		if(rco->tblMain.labelOffset != RCO_NULL_PTR)
			rco->tblMain.labelOffset = rcoxml_add_label_reordering(newLabels, &labelPos, rco->labels + rco->tblMain.labelOffset);
		
		// we re-order in order of IDs
		uint i;
		rRCOEntry** sList = make_sorted_list_of_subentries(&rco->tblMain, label_reorder_qsort);
...
void parse_entry(xmlNodePtr node, rRCOEntry* entry, rRCOFile* rco, rcoxml_read_fixes* fixes) {
	...
	entry->rco = rco;
	entry->parent = entry->firstChild = entry->lastChild = entry->prev = entry->next = NULL;
	entry->labelOffset = RCO_NULL_PTR;
	entry->extra = NULL;
	entry->extraLen = 0;
	entry->srcFile[0] = '\0';
	entry->srcAddr = entry->srcLen = entry->srcLenUnpacked = entry->srcCompression = 0;
	entry->srcBuffer = NULL;
	
	for(i=0; i<RCOXML_TABLE_TAGS_NUM; i++) {
		j=0;
		while(RCOXML_TABLE_TAGS[i][j][0]) {
			if(!xmlStrcmp(node->name, _X(RCOXML_TABLE_TAGS[i][j]))) {
				entry->id = i;
				entry->type = j;
				break;
			}
	...
	if(!entry->id) {
		...
		if(!sscanf((const char*)node->name, "Unknown_%i_%i", (int*)&entry->id, (int*)&entry->type)) {
	...
	{ // label
		xmlChar* label = xmlGetProp(node, _X("name"));
		if(label) {
			entry->labelOffset = rcoxml_add_label(&(rco->labels), &(rco->labelsLen), (char*)label, FALSE);
			xmlFree(label);
		}
	}
	...
	if(knownEntryType) {
		...
		// read src (used by section below this, so there is some importance with this ordering)
		{
			xmlChar* src = xmlGetProp(node, _X("src"));
			if(src) {
				strcpy(entry->srcFile, (const char*)src);
				xmlFree(src);
				
				entry->srcCompression = 0;
				src = xmlGetProp(node, _X("srcRange"));
				if(src) {
					// parse src params
					char compr[50] = "\0";
					if(sscanf((const char*)src, "%i-%i%50s", &(entry->srcAddr), &(entry->srcLen), compr) >= 2) {
						entry->srcLen -= entry->srcAddr;
						entry->srcLenUnpacked = entry->srcLen; // default - assume no compression
						if(compr[0]) {
							// TODO: handle situations where uncompressed size is not specified
							if(sscanf(compr, ";zlib[%u]", &(entry->srcLenUnpacked)) == 1) { // TODO: a temporary fix (prolly need to add more compression algos, since sscanf seems to be rather basic in retrieving %s)
								rcoxml_text_to_int("zlib", RCOXML_TABLE_DATA_COMPRESSION, &(entry->srcCompression));
					...
					entry->srcAddr = 0;
					entry->srcLen = entry->srcLenUnpacked = filesize(entry->srcFile); // won't work if filename contains a '*'
				}
			}
		}
		...
		#define RCOXML_READ_ATTRIB_AS_INT(nd, name, tbl, dest, undefmsg) { \
			xmlChar* __attr = xmlGetProp(nd, _X(name)); \
			if(__attr) { \
		...
		switch(entry->id) {
			case RCO_TABLE_IMG: case RCO_TABLE_MODEL:
				if(entry->type == 1) {
					...
					// default value
					((rRCOImgModelEntry*)entry->extra)->format = (entry->id == RCO_TABLE_IMG ? RCO_IMG_GIM : RCO_MODEL_GMO);
					((rRCOImgModelEntry*)entry->extra)->compression = RCO_DATA_COMPRESSION_NONE;
					
					if(entry->id == RCO_TABLE_IMG) {
						RCOXML_READ_ATTRIB_AS_INT(node, "format", RCOXML_TABLE_IMG_FMT , &((rRCOImgModelEntry*)entry->extra)->format, "No format attribute defined, defaulting to GIM.");
					} else {
						RCOXML_READ_ATTRIB_AS_INT(node, "format", RCOXML_TABLE_MODEL_FMT , &((rRCOImgModelEntry*)entry->extra)->format, "No format attribute defined, defaulting to GMO.");
					}
					RCOXML_READ_ATTRIB_AS_INT(node, "compression", RCOXML_TABLE_DATA_COMPRESSION , &((rRCOImgModelEntry*)entry->extra)->compression, "No compression attribute defined, defaulting to 'uncompressed'.");
					((rRCOImgModelEntry*)entry->extra)->unkCompr = 0;
					xmlChar* unk = xmlGetProp(node, _X("unknownByte"));
					if(unk) {
						sscanf((const char*)unk, "%i", &((rRCOImgModelEntry*)entry->extra)->unkCompr);
						xmlFree(unk);
					}
				}
				break;
			case RCO_TABLE_SOUND:
				if(entry->type == 1) {
					...
					RCOXML_READ_ATTRIB_AS_INT(node, "format", RCOXML_TABLE_SOUND_FMT, (uint*)&(se->format), "No format attribute defined, defaulting to VAG.");
					...
					if(ch > 0)
						se->channels = ch;
					else if(!strcasecmp(entry->srcFile + strlen(entry->srcFile) -4, ".wav") && se->format == RCO_SOUND_VAG)
						se->channels = 0; // we have to fix later
					else
						warning("[line %d] Number of channels either not specified, or no valid value found.", node->line);
					xmlFree(chStr);
					
					
					if(se->channels) {
						se->channelData = (uint32*)malloc(se->channels * sizeof(uint32)*2);
						...
						if(srcPartsX) {
							...
							if(numParts == se->channels) {
								for(i=0; i<se->channels; i++) {
									while(isspace(srcParts[0])) srcParts++; // skip whitespace
									strtrimr(srcParts);
									if(sscanf(srcParts, "%i@%i", (int*)&(se->channelData[i*2]), (int*)&(se->channelData[i*2 +1])) != 2)
										warning("[line %d] Invalid syntax for part '%s'", node->line, srcParts);
									srcParts += strlen(srcParts) +1;
								}
							} else
								warning("[line %d] Number of defined parts does not match number of defined sound channels!", node->line);
								...
								for(i=0; i<se->channels; i++) {
									...
									if(file_exists(srcFile)) {
										...
										if(se->channelData[i*2]) {
											...
											if(fp) {
												srcBufferTmp = realloc(srcBufferTmp, curPos + ALIGN_TO_4(se->channelData[i*2]));
												uint8* bufferPos = (uint8*)(srcBufferTmp) + curPos;
												fileread(fp, bufferPos, se->channelData[i*2]);
												fclose(fp);
											} else
												warning("[line %d] Can't find or open file '%s'", node->line, srcFile);
										}
									} else {
							...
							// err, if there's just one channel, we can just use the srcRange, lol
							se->channelData[0] = entry->srcLen;
							se->channelData[1] = 0;
						} else {
							warning("[line %d] Cannot determine sizes of sound channels!", node->line);
						}
					}
			...
			case RCO_TABLE_TEXT:
				if(entry->type == 1) {
					...
					RCOXML_READ_ATTRIB_AS_INT(node, "language", RCOXML_TABLE_TEXT_LANG, (uint*)&te->lang, "No language attribute defined - RCO may no longer be valid.");
					RCOXML_READ_ATTRIB_AS_INT(node, "format", RCOXML_TABLE_TEXT_FMT, (uint*)&te->format, "No destination format defined - assuming UTF16.");
					
					if(te->format != RCO_TEXT_FMT_UTF16 && !rco->ps3) {
						warning("[line %d] Non UTF-16 text not supported on the PSP.", node->line);
					}
					
					// text indexes
					xmlChar* textEntriesX = xmlGetProp(node, _X("entries"));
					if(textEntriesX) {
						char* textEntries = (char*)textEntriesX;
						te->numIndexes = split_comma_list(textEntries);
						te->indexes = (RCOTextIndex*)malloc(te->numIndexes * sizeof(RCOTextIndex));
						memset(te->indexes, 0, te->numIndexes * sizeof(RCOTextIndex));
						
						for(i=0; i<te->numIndexes; i++) {
							while(isspace(textEntries[0])) textEntries++; // skip whitespace
							strtrimr(textEntries);
							te->indexes[i].labelOffset = rcoxml_add_label(&rco->labels, &rco->labelsLen, textEntries, FALSE);
						...
						if(!(xmlInput = parse_text_xml(entry->srcFile, rco, entry)))
							warning("[line %d] Text entries not specified!  Assuming no entries.", node->line);
					}
						...
						if(srcPartsX) {
							...
							if(numParts == te->numIndexes) {
								for(i=0; i<te->numIndexes; i++) {
									while(isspace(srcParts[0])) srcParts++; // skip whitespace
									strtrimr(srcParts);
									if(sscanf(srcParts, "%i@%i", (int*)&(te->indexes[i].length), (int*)&(te->indexes[i].offset)) != 2)
										warning("[line %d] Invalid syntax for part '%s'", node->line, srcParts);
									srcParts += strlen(srcParts) +1;
								}
							} else
								warning("[line %d] Number of defined parts does not match number of defined text entries!", node->line);
							...
							if(strlen(srcFileFmt) < MAX_FILENAME_LEN) {
								...
								void* srcBufferTmp = malloc(1);
								for(i=0; i<te->numIndexes; i++) {
									sprintf(srcFile, srcFileFmt, rco->labels + te->indexes[i].labelOffset);
									if(file_exists(srcFile)) {
										...
										if(te->indexes[i].length) {
											...
											if(fp) {
												...
												srcBufferTmp = realloc(srcBufferTmp, curPos + ALIGN_TO_4(te->indexes[i].length+bomLen));
												uint8* bufferPos = (uint8*)srcBufferTmp + curPos;
												...
												memset(bufferPos + (te->indexes[i].length), 0, ALIGN_TO_4(te->indexes[i].length+bomLen) - (te->indexes[i].length));
												
												/* uint16 unisig;
												if(te->indexes[i].length >= 2)
													fileread(fp, &unisig, sizeof(unisig));
												if(te->indexes[i].length == 1 || unisig != UNICODE_SIGNATURE) {
													te->indexes[i].length += 2; // add space for trailing null
													fseek(fp, 0, SEEK_SET);
												}
												if(te->indexes[i].length <= 2) {
													...
													srcBufferTmp = realloc(srcBufferTmp, curPos + ALIGN_TO_4(te->indexes[i].length));
													uint8* bufferPos = (uint8*)srcBufferTmp + curPos;
													fileread(fp, bufferPos, te->indexes[i].length-2);
													// add terminating null & any necessary padding
													memset(bufferPos + (te->indexes[i].length-2), 0, ALIGN_TO_4(te->indexes[i].length) - (te->indexes[i].length-2));
												} */
												fclose(fp);
											} else
												warning("[line %d] Can't find or open file '%s'", node->line, srcFile);
							...
							// err, if there's just one text, we can just use the srcRange, lol
							te->indexes[0].length = entry->srcLen;
							te->indexes[0].offset = 0;
						} else {
							warning("[line %d] Cannot determine sizes of text data!", node->line);
						}
					}
			...
			case RCO_TABLE_FONT:
				if(entry->type == 1) {
					...
					xmlChar* val;
					val = xmlGetProp(node, _X("unknownShort1"));
					sscanf((const char*)val, "%i", &rfe->format);
					xmlFree(val);
					val = xmlGetProp(node, _X("unknownShort2"));
					sscanf((const char*)val, "%i", &rfe->compression);
					xmlFree(val);
					val = xmlGetProp(node, _X("unknownInt3"));
					sscanf((const char*)val, "%i", &rfe->unknown);
					xmlFree(val);
					val = xmlGetProp(node, _X("unknownInt4"));
		...
		if(entry->type == 0 || (entry->type == 1 && entry->id == RCO_TABLE_VSMX)) {
			...
			switch(entry->id) {
				case RCO_TABLE_VSMX: shortcutTbl = &(rco->tblVSMX); break;
				case RCO_TABLE_TEXT: shortcutTbl = &(rco->tblText); break;
				case RCO_TABLE_IMG: shortcutTbl = &(rco->tblImage); break;
				case RCO_TABLE_SOUND: shortcutTbl = &(rco->tblSound); break;
				case RCO_TABLE_MODEL: shortcutTbl = &(rco->tblModel); break;
				case RCO_TABLE_FONT: shortcutTbl = &(rco->tblFont); break;
				case RCO_TABLE_OBJ: shortcutTbl = &(rco->tblObj); break;
				case RCO_TABLE_ANIM: shortcutTbl = &(rco->tblAnim); break;
			}
			if(shortcutTbl) {
				if(*shortcutTbl) {
					warning("[line %d] 'Container' tree redefinition (only one should exist in an RCO file).", node->line);
				}
				else {
					*shortcutTbl = entry;
				}
...
uint rcoxml_add_label(char** labels, uint* labelsLen, char* label, Bool eventQuirk) {
	...
	while(p < *labelsLen && (*labels)[p]) {
		if(!strcmp(*labels + p, label))
			return p; // found
		p += strlen(*labels + p) +1;
		p = ALIGN_TO_4(p); // urgh, this is kinda a little dirty, but it works; if we hit a blank 4 bytes, the above line will cause it to go forward by 1, this will align it forward to 4, so, if we've hit some nulls, we're effectively jumping 4 bytes at a time
	}
	...
	*labelsLen = newLen;
/*
	uint curLen = rco->labelsLen;
	uint newLen = rco->labelsLen + strlen(label) + 1;
	
	rco->labels = (char*)realloc(rco->labels, newLen);
	strcpy(curLen, label);
...
uint rcoxml_add_label_reordering(char* newLabels, uint* labelPos, char* label) {
	...
	while(p < *labelPos && newLabels[p]) {
		...
		p = ALIGN_TO_4(p); // dirty, but it works (see note from rcoxml_add_label)
	}
...
void rcoxml_reorder_labels(char* newLabels, uint* labelPos, rRCOFile* rco, rRCOEntry* entry) {
	...
	if(entry->labelOffset != RCO_NULL_PTR)
		entry->labelOffset = rcoxml_add_label_reordering(newLabels, labelPos, rco->labels + entry->labelOffset);
	...
	if(entry->id == RCO_TABLE_TEXT && entry->type == 1) {
		...
		for(i=0; i<extra->numIndexes; i++)
			if(extra->indexes[i].labelOffset != RCO_NULL_PTR)
				extra->indexes[i].labelOffset = rcoxml_add_label_reordering(newLabels, labelPos, rco->labels + extra->indexes[i].labelOffset);
	}
...
void parse_obj_extra(xmlNodePtr node, rRCOEntry* entry) {
	...
	if(entry->type <= RCO_OBJ_EXTRA_LEN_NUM && RCO_OBJ_EXTRA_LEN[entry->type] != -1) {
		uint i = 0, i2 = 0;
		...
		for(i=0, i2=0; (int)i<RCO_OBJ_EXTRA_LEN[entry->type]; i++, i2++) {
			...
			if(RCO_OBJ_EXTRA_NAMES[entry->type][i2][0]) {
				val = xmlGetProp(node, _X(RCO_OBJ_EXTRA_NAMES[entry->type][i2]));
			}
			...
			if(!val) {
				if(RCO_OBJ_EXTRA_NAMES[entry->type][i2][0]) {
					warning("[line %d] Missing attribute '%s', defaulting to 0 / nothing.", node->line, RCO_OBJ_EXTRA_NAMES[entry->type][i2]);
				} else {
					char ts[10] = "\0";
					switch(RCO_OBJ_EXTRA_TYPES[entry->type][i2]) {
						case RCO_OBJ_EXTRA_TYPE_FLOAT: strcpy(ts, "Float"); break;
						case RCO_OBJ_EXTRA_TYPE_INT: strcpy(ts, "Int"); break;
						case RCO_OBJ_EXTRA_TYPE_EVENT: strcpy(ts, "Event"); break;
						case RCO_OBJ_EXTRA_TYPE_IMG: strcpy(ts, "Image"); break;
						case RCO_OBJ_EXTRA_TYPE_FONT: strcpy(ts, "Font"); break;
						case RCO_OBJ_EXTRA_TYPE_MODEL: strcpy(ts, "Model"); break;
						case RCO_OBJ_EXTRA_TYPE_OBJ: strcpy(ts, "Object"); break;
						case RCO_OBJ_EXTRA_TYPE_UNK: case RCO_OBJ_EXTRA_TYPE_REF:
							if(isRef) strcpy(ts, "Ref");
							break;
					}
					warning("[line %d] Missing attribute 'unknown%s%d', defaulting to 0 / nothing.", node->line, ts, i);
				}
				...
				if(isRef) {
					...
					if(!rcoxml_parse_ref((char*)val, (rRCORef*)extra))
						warning("[line %d] Unable to parse reference '%s' - defaulting to 'nothing'.", node->line, (char*)val);
					/*
					// we really need to fix refs after all entries are loaded - I'm going to be lazy here since we can't actually do anything terribly useful right now
					((rRCORef*)extra)->type = RCO_REF_NONE;
...
void parse_anim_extra(xmlNodePtr node, rRCOEntry* entry) {
	...
	if(entry->type <= RCO_ANIM_EXTRA_LEN_NUM && RCO_ANIM_EXTRA_LEN[entry->type] != -1) {
		uint i = 0, i2 = 0;
		...
		for(i=0, i2=0; (int)i<RCO_ANIM_EXTRA_LEN[entry->type]; i++, i2++) {
			...
			if(RCO_ANIM_EXTRA_NAMES[entry->type][i2][0]) {
				val = xmlGetProp(node, _X(RCO_ANIM_EXTRA_NAMES[entry->type][i2]));
			}
			...
			if(!val) {
				if(RCO_ANIM_EXTRA_NAMES[entry->type][i2][0]) {
					warning("[line %d] Missing attribute '%s', defaulting to 0 / nothing.", node->line, RCO_ANIM_EXTRA_NAMES[entry->type][i2]);
				} else {
					char ts[10] = "\0";
	...
	if(entry->type <= RCO_ANIM_EXTRA_LEN_NUM && RCO_ANIM_EXTRA_LEN[entry->type] != -1) {
		...
		for(i=0, i2=0; i<(uint)RCO_ANIM_EXTRA_LEN[entry->type]; i++, i2++) {
			if(RCO_ANIM_IS_REF(entry->type, i2)) {
				...
				entry->extraLen += sizeof(uint32);
		}
		
		//if(RCO_ANIM_EXTRA_REFS[entry->type]) {
		//	entry->extraLen = (RCO_ANIM_EXTRA_LEN[entry->type]-2) * sizeof(uint32) + sizeof(rRCORef);
		//} else {
		//	entry->extraLen = RCO_ANIM_EXTRA_LEN[entry->type] * sizeof(uint32);
		//}
		...
		if(RCO_ANIM_EXTRA_REFS[entry->type]) {
			xmlChar* val = NULL;
			if(entry->type == RCO_ANIM_TYPE_EVENT)
				val = xmlGetProp(node, _X("event"));
			else
				val = xmlGetProp(node, _X("object"));
			
			if(val) {
				if(!rcoxml_parse_ref((char*)val, (rRCORef*)extra))
					warning("[line %d] Unable to parse object reference '%s' - defaulting to 'nothing'.", node->line, (char*)val);
			} else {
				warning("[line %d] No object/event defined for anim entry.  Defaulting to nothing.", node->line);
				// blank ref
				rRCORef* dr = ((rRCORef*)extra);
				dr->type = RCO_REF_NONE;
				dr->ptr = NULL;
				dr->rawPtr = RCO_NULL_PTR;
			}
			extra += sizeof(rRCORef);
			entryLen -=2;
		}
		...
		for(i=0; i<entryLen; i++) {
			xmlChar* val = NULL;
			if(RCO_ANIM_EXTRA_NAMES[entry->type][i][0]) {
				val = xmlGetProp(node, _X(RCO_ANIM_EXTRA_NAMES[entry->type][i]));
			}
			if(!val) {
				val = rcoxml_get_unknown_attrib(node, i);
			}
			...
			if(!val) {
				if(RCO_ANIM_EXTRA_NAMES[entry->type][i][0]) {
					warning("[line %d] Missing attribute '%s', defaulting to 0 / nothing.", node->line, RCO_ANIM_EXTRA_NAMES[entry->type][i]);
				} else {
					warning("[line %d] Missing attribute (index %d), defaulting to 0 / nothing.", node->line, i); // TODO: better message
				}
				*(uint32*)extra = 0;
			}
			else {
...
// TODO: this somewhat mixes normal vals with refs (not optimal) - may wish to do something about this
xmlChar* rcoxml_get_unknown_attrib(xmlNodePtr node, uint num) {
	xmlChar* ret = NULL;
	char n[30];
...
uint32 rcoxml_parse_value(char* s) {
	...
	if(sscanf(s, "0x%x", (uint*)&retI)) return retI;
...
Bool rcoxml_parse_ref(char* val, rRCORef* out) {
	...
	if(out->type == RCO_REF_NONE) { // not assigned by above ifs
		if(sscanf(val, "unknown%i", (int*)&(out->type))) { // this is actually case sensitive... :|
...
/*
// a very simple resize method :/
void add_ref_to_fix(rcoxml_read_fixes* fixes, char* label, rRCORef* ref) {
	fixes->refs = (xmlrco_read_fix_refs*)realloc(fixes->refs, sizeof(xmlrco_read_fix_refs) * (fixes->refCnt+1));
	xmlrco_read_fix_refs* fr = &(fixes->refs[fixes->refsCnt]);
	
	fr->ref = ref;
	fr->label = (char*)malloc(strlen(label)+1);
	strcpy(fr->label, label);
...
void rcoxml_fix_refs(rRCOEntry* entry, rRCOFile* rco) {
	...
	if((entry->id == RCO_TABLE_OBJ && entry->type > 0 && entry->type <= RCO_OBJ_EXTRA_LEN_NUM && RCO_OBJ_EXTRA_LEN[entry->type] != -1)
	|| (entry->id == RCO_TABLE_ANIM && entry->type > 1 && entry->type <= RCO_ANIM_EXTRA_LEN_NUM && RCO_ANIM_EXTRA_LEN[entry->type] != -1)) {
		uint8* extra = (uint8*)entry->extra;
...
Bool rcoxml_fix_ref(rRCORef* ref, rRCOFile* rco) {
	if(ref->type == RCO_REF_IMG || ref->type == RCO_REF_MODEL || ref->type == RCO_REF_FONT || ref->type == RCO_REF_OBJ2 || ref->type == RCO_REF_ANIM || ref->type == RCO_REF_OBJ) {
		if(!ref->ptr) return FALSE; // no label... (this should never happen...)
		
		rRCOEntry* e = find_entry_from_label(&(rco->tblMain), (const char*)ref->ptr);
		if(!e) {
			warning("[entry-reference] Cannot find entry with label '%s'!", (const char*)ref->ptr);
		}
		else if(
		     (ref->type == RCO_REF_IMG && e->id != RCO_TABLE_IMG)
		  ...
		  || (ref->type == RCO_REF_ANIM && e->id != RCO_TABLE_ANIM)
			...
			warning("[entry-reference] Mismatching reference type and entry type for reference with label '%s' - have you used the right name?", (const char*)ref->ptr);
		}
		...
		if(!ref->ptr) return FALSE; // no event... (this should never happen...)
		
		ref->rawPtr = rcoxml_add_label(&(rco->events), &rco->eventsLen, (char*)ref->ptr, TRUE);
		free(ref->ptr); // unallocate malloc'd label
		return TRUE;
	}
		...
		if(rco->tblText && rco->tblText->numSubentries) {
			int idx = find_text_from_label(rco->labels, (rRCOTextEntry*)(rco->tblText->firstChild->extra), (const char*)ref->ptr);
			if(idx == -1) {
				warning("[entry-reference] Cannot find text entry with label '%s'! (note, only first text language is checked as the RCO format assumes that all languages have the same entry labels)", (const char*)ref->ptr);
			} else {
				ref->rawPtr = idx;
			}
		} else
			warning("[entry-reference] Unable to reference text entry '%s' as text tree cannot be found, or contains no text languages!", (const char*)ref->ptr);
...
void rcoxml_fix_ptrs(rRCOEntry*** sect, uint* sectCnt, rRCOFile* rco, const char* text) {
	...
	*sect = (rRCOEntry**)malloc(*sectCnt * sizeof(rRCOEntry*));
	...
	for(i=0; i<*sectCnt; i++) {
		while(isspace(tmpTextPtr[0])) tmpTextPtr++; // skip whitespace
		strtrimr(tmpTextPtr);
...
// takes a comma separated string, replaces commas with nullchars and returns number of items in the list
uint split_comma_list(char* s) {
	uint cnt = 1;
	uint i;
	uint sLen = strlen(s);
...
Bool parse_text_xml(char* fn, rRCOFile* rco, rRCOEntry* entry) {
	...
	if((node = xmlDocGetRootElement(doc)))
		bValidDoc = (xmlStrcmp(node->name, _X("TextLang")) ? FALSE : TRUE);
	if(!bValidDoc) {
	...
	// count num of child nodes (don't use xmlChildElementCount because this may include text nodes)
	xmlNodePtr np = node->xmlChildrenNode;
	while(np->next) {
		if(np->type == XML_ELEMENT_NODE && !xmlStrcmp(np->name, _X("Text")))
			te->numIndexes++;
		np = np->next;
	}
	...
	if(te->numIndexes) {
		...
		te->indexes = (RCOTextIndex*)calloc(1, te->numIndexes * sizeof(RCOTextIndex));
		...
		while(np->next) {
			if(np->type == XML_ELEMENT_NODE && !xmlStrcmp(np->name, _X("Text"))) {
				xmlChar* n = xmlGetProp(np, _X("name"));
				if(n) {
					te->indexes[i].labelOffset = rcoxml_add_label(&rco->labels, &rco->labelsLen, (char*)n, FALSE);
					xmlFree(n);
					
					te->indexes[i].length = 0;
					te->indexes[i].offset = RCO_NULL_PTR;
					
					if(np->xmlChildrenNode && (np->xmlChildrenNode->type == XML_TEXT_NODE || np->xmlChildrenNode->type == XML_CDATA_SECTION_NODE)) {
						...
						if(fmt == RCO_TEXT_FMT_UTF8) {
							...
							te->indexes[i].length = (xmlUTF8Strlen(n)+1 /*null*/) * charWidth;
							if((*(uint32*)n & 0xFFFFFF) == UTF8_BOM) { // fix for older versions of rcomage - if BOM exists, reduce length by 1 char
								te->indexes[i].length -= charWidth;
							}
						}
						if(te->indexes[i].length > 2) {
							size_t contentLen = xmlStrlen(n), outBufLen = te->indexes[i].length;
							
							textBuffer = realloc(textBuffer, ALIGN_TO_4(te->indexes[i].length + curPos));
							char* tbPtr = (char*)textBuffer;
							tbPtr += curPos;
							// first get rid of BOM if we have one (earlier versions of rcomage)
							/* if((fmt == RCO_TEXT_FMT_UTF32 && *(uint32*)n == UTF32_BOM)
							 ||(fmt == RCO_TEXT_FMT_UTF16 && *(uint16*)n == UTF16_BOM)
							 ||(fmt == RCO_TEXT_FMT_UTF8 && (*(uint32*)n & 0x00FFFFFF) == UTF8_BOM)) { */
							if((*(uint32*)n & 0xFFFFFF) == UTF8_BOM) {
								char bom[4];
								char *bomPtr = bom;
								size_t bomLen = (fmt == RCO_TEXT_FMT_UTF32 ? 4 : (fmt == RCO_TEXT_FMT_UTF8 ? 3 : 2));
								iconv(ic, (char**)(&n), &contentLen, (char**)&bomPtr, &bomLen);
							}
							iconv(ic, (char**)(&n), &contentLen, &tbPtr, &outBufLen);
							if(outBufLen && outBufLen == charWidth) { // *should* always be true
								...
								warning("[%s:%d] iconv error when trying to convert text for '%s'! (%d byte(s) not converted)", fn, np->line, rco->labels + te->indexes[i].labelOffset, outBufLen - charWidth);
