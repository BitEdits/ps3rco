...
FILE* fopen_local(char* fn, char* mode) {
	...
	if(configDir ||
	  ...
	  (is_dir(RCOMAGE_DATADIR) && (configDir = RCOMAGE_DATADIR)) ) {
		...
		// solution to get current path from http://stackoverflow.com/questions/143174/c-c-how-to-obtain-the-full-path-of-current-directory
#ifdef WIN32
		bytes = GetModuleFileName(NULL, path, 512);
		if(bytes) p = strrchr(path, '\\');
#else
		char szTmp[32];
		sprintf(szTmp, "/proc/%d/exe", getpid());
		bytes = readlink(szTmp, path, 512);
...
void configLoadTagmap(void) {
	...
	if(fp) {
		...
		while((type = get_ini_line(fp, buf, &key, &val))) {
			if(type == 1) {
				id++;
				RCOXML_TABLE_TAGS = (RcoTagMap)realloc(RCOXML_TABLE_TAGS, sizeof(*RCOXML_TABLE_TAGS) * (id+2));
				RCOXML_TABLE_TAGS[id][0][0] = 0;
				id2 = -1;
				RCOXML_TABLE_NAMES = (RcoTableMap)realloc(RCOXML_TABLE_NAMES, sizeof(*RCOXML_TABLE_NAMES) * (id+1));
...
void configLoadMiscmap(void) {
	...
	if(fp) {
		...
		fclose(fp);
		#define CONFIG_LOAD_MISC_CHECK(v, s) if(!v) { error("Couldn't load " s " constants from config file!"); exit(1); }
...
void configLoadObjmap(Bool ps3) {
	...
	if(ps3) strcpy(from, "objattribdef-ps3.ini");
...
void configLoadAnimmap(Bool ps3) {
	...
	if(ps3) strcpy(from, "animattribdef-ps3.ini");
	FILE* fp = fopen_local(from, "r");
	if(fp) {
		char buf[INI_LINE_BUF_LEN];
		char *key, *val;
		...
		while((type = get_ini_line(fp, buf, &key, &val))) {
			if(type == 1) {
				id++;
				RCO_ANIM_EXTRA_LEN = (int*)realloc(RCO_ANIM_EXTRA_LEN, sizeof(int) * (id+1));
				RCO_ANIM_EXTRA_LEN[id] = 0;
				RCO_ANIM_EXTRA_NAMES = (RcoObjMap)realloc(RCO_ANIM_EXTRA_NAMES, sizeof(*RCO_ANIM_EXTRA_NAMES) * (id+1));
				...
				if(RCO_ANIM_IS_REF(id, id2)) RCO_ANIM_EXTRA_LEN[id]++;
			}
		}
		fclose(fp);
